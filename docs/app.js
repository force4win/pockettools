const app = {
    ...
    renderToolInterface(toolId) {
        const template = document.getElementById(`tool-${toolId}`);

        if (template) {
            this.dom.toolContent.innerHTML = '';
            this.dom.toolContent.appendChild(template.content.cloneNode(true));
        } else {
            console.error(`Template tool-${toolId} not found`);
            this.dom.toolContent.innerHTML = '<p>Error cargando interfaz.</p>';
        }
